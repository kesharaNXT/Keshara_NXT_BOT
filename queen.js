...
async function sessdl() {
  try {
    ...
    const base64Data = sess.SESSION_ID.split("KESHARA-NXT~")[1];
    if (!base64Data) {
  ...
  conn.ev.on("connection.update", (update) => {
    ...
    if (connection === "close") {
      ...
      conn.sendMessage(conn.user.id, {
        text: up,
        contextInfo: {
          mentionedJid: ["947616645979@s.whatsapp.net"], // specify mentioned JID(s) if any
          groupMentions: [],
          forwardingScore: 999,
          isForwarded: true,
          forwardedNewsletterMessageInfo: {
            newsletterJid: "120363299978149557@newsletter",
            newsletterName: "© KESHARA_NXT 💚",
            serverMessageId: 999,
          },
          externalAdReply: {
            title: "© KESHARA_NXT 💚",
